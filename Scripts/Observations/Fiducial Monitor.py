...
class ScriptClass(object):
    def __init__(self, sr):
        ...
        self.azMax = sr.getKeyVar(self.mcpModel.msOnMaxCorrection, ind=0,
                                  defVal=600)
        self.altMax = sr.getKeyVar(self.mcpModel.msOnMaxCorrection, ind=1,
                                   defVal=600)
        self.rotMax = sr.getKeyVar(self.mcpModel.msOnMaxCorrection, ind=2,
                                   defVal=600)
        ...
        if abs(azFid0[3]) > self.azMax:
            self.logWdg1.addMsg("%s ( > %s)" % (self.largeMes, str(self.azMax)),
                                severity=self.blueWarn)
            self.warning()
        ...
        self.mcpModel.azFiducialCrossing.addCallback(self.updateAz,
                                                     callNow=False)
        self.mcpModel.altFiducialCrossing.addCallback(self.updateAlt,
                                                      callNow=False)
        self.mcpModel.rotFiducialCrossing.addCallback(self.updateRot,
                                                      callNow=False)

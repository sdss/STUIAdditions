...
class ScriptClass(object):
    def __init__(self, sr):
        ...
        self.but0 = RO.Wdg.Button(master=F0, helpText=helpText[0],
                                  callFunc=self.saveOff,
                                  text=butList[0], ).grid(row=row, column=0)
        self.but1 = RO.Wdg.Button(master=F0, helpText=helpText[1],
                                  callFunc=self.applOff,
                                  text=butList[1], ).grid(row=row, column=1)
        self.but2 = RO.Wdg.Button(master=F0, helpText=helpText[2],
                                  callFunc=self.clearOff,
                                  text=butList[2], ).grid(row=row, column=2)
        row += 1

        F1 = Tkinter.Frame(master=sr.master)
        F1.grid(row=row, column=0, sticky="w", )
        self.but3 = RO.Wdg.Button(master=F1, helpText=helpText[3],
                                  callFunc=self.gotofk5to3,
                                  text=butList[3], ).grid(row=row, column=0)
        self.but4 = RO.Wdg.Button(master=F1, helpText=helpText[4],
                                  callFunc=self.gotofk5to11,
                                  text=butList[4], ).grid(row=row, column=1)
        row += 1
    ...
    def run(self, sr):
        ...
        self.logWdg.addMsg(
            "%s   %s,   objArcOff = %s" % (tm, self.getCart(sr, ), ss0))

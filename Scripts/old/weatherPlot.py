...
class ScriptClass(object):
    def __init__(self, sr):
     #   sr.debug = True  # if True, run in debug-only mode 
        ...
        timeRange=7200; width=8; height=4.6
   ...
   # make plot resizable     
        ...
        self.ppWdg.xaxis.set_major_locator(matplotlib.dates.MinuteLocator(byminute=range(0,60,15)))
        ...
        try:  sr.getKeyVar(airtemp, ind=0)
        except:  airtemp0=0.0
        else: airtemp0=sr.getKeyVar(airtemp, ind=0)
        finally: pass
        
        dewpoint=self.apoModel.dpTempPT
        self.ppWdg.plotKeyVar(label="diff25m", subplotInd=0, keyVar=dewpoint, func=diff, keyInd=0, color="red")
        self.ppWdg.plotKeyVar(label="airtemp", subplotInd=0, keyVar=airtemp, keyInd=0, color="blue")
        self.ppWdg.plotKeyVar(label="dewpoint", subplotInd=0, keyVar=dewpoint, keyInd=0, color="green")
        ...
        try:  sr.getKeyVar(winds25m, ind=0)
        except:  winds25m0=0.0
        else: winds25m0=sr.getKeyVar(winds25m, ind=0)
        finally: pass        
        self.ppWdg.plotKeyVar(label="winds25m", subplotInd=pl, keyVar=winds25m, keyInd=0, color="red")
        winds35m=self.apoModel.winds
        self.ppWdg.plotKeyVar(label="winds35m", subplotInd=pl, keyVar=winds35m, keyInd=0, color="blue")
        gusts35m=self.apoModel.gusts
        self.ppWdg.plotKeyVar(label="gusts35m", subplotInd=pl, keyVar=gusts35m, keyInd=0, color="green")        

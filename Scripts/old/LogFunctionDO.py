...
class ScriptClass(object):
    def __init__(self, sr):
        ...
        #sop
        self.sopModel.gotoFieldState.addCallback(self.updateGotoField, callNow = False)
...
#                self.logWdg2.addMsg("%s" % (95*""))
                ...
                for spNum in self.spNumList:
                    ...
                    self.logWdg2.addMsg("    sp%s offset: r = %s steps (%s);  b = %s deg (%s) " % \
                           (spNum, rPiston, rStr, bRing, bStr), tags = "c")
                for spNum in self.spNumList:
                    self.logWdg2.addMsg("    sp%s pred. move: spAverageMove = %s steps" % \
                ...
                for spNum in self.spNumList:
                    ...
                    bRing = self.bRingMove[spNum]
                    bStr = self.bMeanOffset[spNum]
                    self.logWdg1.addMsg("    sp%s offset: r = %s steps (%s);  b = %s deg (%s) " % \
                           (spNum, rPiston, rStr, bRing, bStr), tags = "c")
                for spNum in self.spNumList:
                    self.logWdg1.addMsg("    sp%s pred. move: spAverageMove = %s steps" % \
                           (spNum, self.avrMove[spNum]), tags = "c")
                for spNum in self.spNumList:
                    spRes = self.resid[spNum]
                    spTemp = getattr(self.bossModel, "sp%sTemp" % (spNum))
            ...
            if (keyVar[1] != ":") and ("MaNGA" in survey):   # If command finished with status of fail.
                self.logWdg2.addMsg("  Hartmann failed.", severity = self.redWarn)
            if (keyVar[1] != ":") and ("eBOSS" in survey):   # If command finished with status of fail.
...
#                dd3 = self.apogeeModel.exposureState[2]
            ...
            ss = "  %s  apogee exposure %s, expName=%s, nReads=%s, dither=%s" % (timeStr, exptp, dd0, dd3, dth)
            self.logWdg3.addMsg("%s" % (ss), tags="c")

    # Update BOSS exposure and print appropriate info to the appropriate log window.
    def updateBossState(self, keyVar): # keyVar is equal to the bossModel.exposureState keyword.
        if not keyVar.isGenuine: return # Exclude stale keyword value.
        timeStr = self.getTAITimeStr()
        if keyVar[0] != self.expState: # If keyword is different (something has changed) then proceed.
            ...
            if self.expState == "INTEGRATING": # This is valid for biases, too: exposureState=INTEGRATING,0.0,0.0
                ...
                if "MaNGA" in survey:
                    ...
                    if self.mcpModel.ffLamp[0] == 1:
                        ss = "  %s  boss exposure flat, expNum=%i, expTime=%s" % (timeStr, expId, expTime)
                        self.logWdg2.addMsg("%s " % (ss), tags="g")
                    elif self.mcpModel.hgCdLamp[0] == 1:
                        ss = "  %s  boss exposure  arc, expNum=%i, expTime=%s" % (timeStr, expId, expTime)
                        self.logWdg2.addMsg("%s " % (ss), tags="g")
                    elif expTime == 900.0:
                        ss = "  %s  boss exposure science, expNum=%i, expTime=%s" % (timeStr, expId, expTime)
                        self.logWdg2.addMsg("%s, %s" % (ss,mangaDith), tags="g")
                    elif expTime == 0.:
                        ss = "  %s  boss exposure bias, expNum=%i, expTime=%s" % (timeStr, expId, expTime)
                        self.logWdg2.addMsg("%s " % (ss), tags="g")
                    else:
                        ss = "  %s  boss exposure dark, expNum=%i, expTime=%s" % (timeStr, expId, expTime)
                        self.logWdg2.addMsg("%s " % (ss), tags="g")
    ...
    def updateLoadCart(self, keyVar):
        ...
        sd = keyVar[2] # Parse the side ('A' or 'B'; obsolete - MARVELS) from keyVar.
        if [ct, pl, sd] != self.loadCart:
            self.updateLoadCartOutput()
            self.loadCart = [ct, pl, sd] # Update self.loadCart with new cartridge, plate, side.
            self.survey = ''
            self.surveyLead = ''
    ...
    def updateGotoField(self, keyVar):
        ...
        if keyVar[0] != self.gotoFieldState:
            ...
            if keyVar[0] == 'running':
                if "MaNGA" in self.survey:
                    self.logWdg2.addMsg("%sZ - gotoField. noGSOGTF. Clear. Seeing:  FWHM" % (timeStr))
                    self.logWdg2.addMsg("%s" % (95*""))
                if "APOGEE" in self.survey:
                    self.logWdg3.addMsg("%sZ - gotoField. noGSOGTF. Clear. Seeing:  FWHM" % (timeStr))
                    self.logWdg3.addMsg("%s" % (95*""))
                else:
                    self.logWdg1.addMsg("%sZ - gotoField. noGSOGTF. Clear. Seeing:  FWHM" % (timeStr))
                    self.logWdg1.addMsg("%s" % (95*""))
#            if keyVar[0] == 'failed':
#                if "MaNGA" in self.survey:

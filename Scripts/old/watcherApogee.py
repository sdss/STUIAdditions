...
class ScriptClass(object):
    def __init__(self, sr, ):
        ...
        row = 0
        self.labWdg0 = RO.Wdg.Label(master=sr.master,
                                    text="0: Apogee exposure time left")
        ...
        row = row + 1  # 2
        self.labWdg2 = RO.Wdg.Label(master=sr.master,
                                    text="sop.doApogeeScience_sequenceState=")
        self.labWdg2.grid(row=row, column=0, sticky="nsw")
        self.seqStateVar = sr.getKeyVar(
            self.sopModel.doApogeeScience_sequenceState, ind=0, defVal=None)

        row = row + 1  # 3
        self.labWdg3 = RO.Wdg.Label(master=sr.master,
                                    text="doApogeeScience_seqCount= ")
        self.labWdg3.grid(row=row, column=0, sticky="nsw")
        #    self.doApogeeScience_seqCount=self.sopModel.doApogeeScience_seqCount[0]
        self.doApScience_seqCount = sr.getKeyVar(
            self.sopModel.doApogeeScience_seqCount, ind=0, defVal=None)

        row = row + 1
        self.labWdg4 = RO.Wdg.Label(master=sr.master,
                                    text="sop.doApogeeScience_ditherSeq =")
        self.labWdg4.grid(row=row, column=0, sticky="nsw")
        self.seqStateVar = sr.getKeyVar(self.sopModel.doApogeeScience_ditherSeq,
                                        ind=0, defVal=None)
        ...
        # ------------------
        self.warnTimeA = 5;
        self.warn5A = False  # Apogee

        self.record(sr)  # #0
        self.mcpModel.apogeeGang.addCallback(self.getGang, callNow=True)  # 1
        self.sopModel.doApogeeScience_sequenceState.addCallback(
            self.doApogeeScience_sequenceState, callNow=True)  # 2
        self.sopModel.doApogeeScience_seqCount.addCallback(
            self.doApogeeScience_seqCount, callNow=True)  # 3
        self.sopModel.doApogeeScience_ditherSeq.addCallback(
            self.doApogee_dither, callNow=True)  # 4
        self.apogeeModel.utrReadState.addCallback(self.utrState,
                                                  callNow=True)  # 5
        self.sopModel.doApogeeScience_expTime.addCallback(
            self.doApogeeScience_expT, callNow=True)  # 6
        self.sopModel.doApogeeScienceStages.addCallback(self.apSatges,
                                                        callNow=True)  # 7
        self.sopModel.doApogeeScienceState.addCallback(self.apState,
                                                       callNow=True)  # 8
        self.apogeeModel.utrReadTime.addCallback(self.utrExpTime,
                                                 callNow=True)  # 9
    ...
    def utrState(self, keyVar):
        self.labWdg5.set("5: apogee.utrState = %s, %s, %s, %s" % (
        keyVar[0], keyVar[1], keyVar[2], keyVar[3]))
        self.record()
    ...
    def doApogeeScience_seqCount(self, keyVar):
        self.labWdg3.set(
            "3: sop.doApogeeScience_seqCount[0] = %s" % (keyVar[0]))
        self.record()

    # --2------
    def doApogeeScience_sequenceState(self, keyVar):
        self.labWdg2.set("2: sop.doApogeeScience_sequenceState = %s, %s" % (
        keyVar[0], keyVar[1]))
        self.record()

...
class ScriptClass(object):
    def __init__(self, sr, ):
        ...
        self.checkWdg = RO.Wdg.Checkbutton(master=F1, text="", defValue=True,
                                           helpText="Play sound", )
        ...
        self.expTotal = sr.getKeyVar(self.sopModel.doBossScience_expTime, ind=0,
                                     defVal=900)
        ...
        self.sopModel.doApogeeMangaSequence_ditherSeq.addCallback(
            self.doScience_nExp, callNow=True)

    def getTAITimeStr(self, ):
        """ get timestamp"""
        self.currPythonSeconds = RO.Astro.Tm.getCurrPySec()
        self.currTAITuple = time.gmtime(
            self.currPythonSeconds - RO.Astro.Tm.getUTCMinusTAI())
    ...
    def doScience_nExp(self, keyVar):
        """callback function if the number of sop done or scheduler exposures
        changed """
        sr = self.sr
        self.expTotal = sr.getKeyVar(self.sopModel.doBossScience_expTime, ind=0,
                                     defVal=900)

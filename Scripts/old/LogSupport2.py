...
class ScriptClass(object, ):
    def __init__(self, sr, ):
        ...
        ss = "Time %s Inst    Temp   DP   Dif  Humid Wind Dir Dust,1um IRSC  IRSCm <fwhm>" % (
        2 * s,)
        ...
        self.sopModel = TUI.Models.getModel('sop')
        self.mangaIndex = self.sopModel.doMangaSequence_ditherSeq[1]
        # self.sopModel.doMangaSequence_ditherSeq.addCallback(self.updateMangaState, callNow=True)
    ...
    def print_hartmann_to_log(self):
        ...
        try:
            ss2 = "%5i %5.1f %5i %5.1f %4.1f" % (
            rPiston, bRing, spAvMove, spRes, spTemp)
        except Exception:
            ss2 = "%5s %5s %5s %5s %4s" % (
            rPiston, bRing, spAvMove, spRes, spTemp)
    ...
    def updateMangaState(self, keyVar):
        if not keyVar.isGenuine: return
        if keyVar[1] != self.mangaIndex:
            sr = self.sr
            self.record(sr, "manga")
            self.mangaIndex = keyVar[1]
    ...
    def record(self, sr, atm):
        ...
        primOr = self.fInt(
            sr.getKeyVar(self.tccModel.primOrient, ind=0, defVal=9999), 5)
        secOr = self.fInt(
            sr.getKeyVar(self.tccModel.secOrient, ind=0, defVal=9999), 5)
        secFoc = self.fInt(
            sr.getKeyVar(self.tccModel.secFocus, ind=0, defVal=9999), 4)
        ...
        objOff0 = ffsec(
            RO.CnvUtil.posFromPVT(self.tccModel.objArcOff[0]))  # *3600.0
        objOff1 = ffsec(
            RO.CnvUtil.posFromPVT(self.tccModel.objArcOff[1]))  # *3600.0

        guideOff0 = ffsec(
            RO.CnvUtil.posFromPVT(self.tccModel.guideOff[0]))  # *3600.0
        guideOff1 = ffsec(
            RO.CnvUtil.posFromPVT(self.tccModel.guideOff[1]))  # *3600.0
        guideOff2 = ffsec(
            RO.CnvUtil.posFromPVT(self.tccModel.guideOff[2]))  # *3600.0

        calibOff0 = ffsec(
            RO.CnvUtil.posFromPVT(self.tccModel.calibOff[0]))  # *3600.0
        calibOff1 = ffsec(
            RO.CnvUtil.posFromPVT(self.tccModel.calibOff[1]))  # *3600.0
        calibOff2 = ffsecS(
            RO.CnvUtil.posFromPVT(self.tccModel.calibOff[2]))  # *3600.0
        ...
        humid = self.fInt(
            sr.getKeyVar(self.apoModel.humidPT, ind=0, defVal=999), 3)
        dustb = self.fInt(sr.getKeyVar(self.apoModel.dustb, ind=0, defVal=9999),
                          5)
        ...
        ss2 = "(%2.0f,%2.0f,%2.0f) " % (
        float(calibOff0), float(calibOff1), float(calibOff2))
        ...
        ss2 = " %6.1f  %4.1f  %5.1f  %s  %s  %3.1f %s" % (
        az, alt, airT, wind, dir, fwhm, atm)
        ss = ss1 + ss2
        self.logWdg2.addMsg("%s" % (ss), tags=["g", "cur"])

        # weather
        ss1 = "%s %s %5.1f %5.1f %5.1f  %s" % (tm, cart, airT, dp, diff, humid,)
        ss2 = "   %s  %s  %s  %5.1f  %4i  %3.1f %s" % (
        wind, dir, dustb, irsc, irscmean, fwhm, atm)
        ss = ss1 + ss2
        self.logWdg3.addMsg("%s " % (ss), tags=["cur"])

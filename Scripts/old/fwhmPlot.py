...
class ScriptClass(object):
    def __init__(self, sr):
        ...
        self.ppWdg.xaxis.set_major_locator(
            mpl.dates.MinuteLocator(byminute=range(0, 61, 10)))

        self.probe = self.guiderModel.probe

        def fwhmFun(val):
            try:
                sr.getKeyVar(self.probe, ind=6)
            except:
                self.focOff = 0
            else:
                self.focOff = sr.getKeyVar(self.probe, ind=6)
            finally:
                pass
            if sefl.focOff == 0:
                val = val
        ...
        self.ppWdg.plotKeyVar(label="seeing", subplotInd=0, keyVar=self.seeing,
                              keyInd=0, color="red")
        self.ppWdg.subplotArr[0].legend(loc=3, frameon=False)

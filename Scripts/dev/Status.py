...
02-20-2013 changed time to tai; add date to initial time stamps, color title of
  other section
09-09-2013 EM:
...
class ScriptClass(object):
    ...
    def getTAITimeStr():
        ...
        return taiTimeStr, taiDateStr, currTAITuple
    ...
    def run(self, sr):
        ...
        mcpWs = str(mcpW[0]) + "," + str(mcpW[1]) + "," + str(
            mcpW[2]) + "," + str(mcpW[3])
        self.logWdg.addMsg("cwPositions =  %s  %s  %s  %s;  " %
                           (str(mcpW[0]), str(mcpW[1]), str(mcpW[2]),
                            str(mcpW[3])))
        ...
        ss = "Guider cart= %s,  plate= %s,  side= %s (loaded)" % (
            str(gc), str(gp), str(gs))
        if mcpInst != gc:
            self.logWdg.addMsg("%s;  " % ss, severity=self.redWarn)
        else:
            self.logWdg.addMsg("%s;  " % ss, severity=0)
        self.logWdg.addMsg("%s" % ('-' * self.line))

        motPos = [0] * 3
        for i in range(len(motPos)):
            motPos[i] = sr.getKeyVar(bossModel.motorPosition, ind=i,
                                     defVal=defstr)
        self.logWdg.addMsg("motorPosition", tags=["b"])

        self.logWdg.addMsg("%s sp1:  %6s,  %6s,  %6s;  " % (
            " " * 4, motPos[0], motPos[1], motPos[2]))
        # self.logWdg.addMsg("%s sp2:  %6s,  %6s,  %6s;  " % (
        # " " * 4, motPos[3], motPos[4], motPos[5]))
        self.logWdg.addMsg("%s" % ('-' * self.line))

        yield sr.waitCmd(actor="boss", cmdStr="status",
                         keyVars=[bossModel.sp1Temp],  # , bossModel.sp2Temp],
                         checkFail=False)
        if sr.value.didFail:
            sp1T = "n/a"
            # sp2T = "n/a"
        ...
        self.logWdg.addMsg(
            "%s Outside air temperature= %s;  " % (" " * 4, str(airT)))
        ...
        self.logWdg.addMsg("    sp1r0= %s  (%s);  " % (sss(sp1r0), sss(sp1r0N)),
                           severity=sevf(sp1r0, sp1r0N, 5))
        self.logWdg.addMsg("    sp1b2= %s  (%s);  " % (sss(sp1b2), sss(sp1b2N)),
                           severity=sevf(sp1b2, sp1b2N, 5))
        # self.logWdg.addMsg("    sp2r0= %s  (%s);  " % (sss(sp2r0),
        # sss(sp2r0N)),
        # severity=sevf(sp2r0, sp2r0N, 5))
        # self.logWdg.addMsg("    sp2b2= %s  (%s);  " % (sss(sp2b2),
        # sss(sp2b2N)),
        #                    severity=sevf(sp2b2, sp2b2N, 5))
        ...
        if SP1SecDewPress == defstr:
            ...
            self.logWdg.addMsg(
                "%s sp1LN2: %4.1f;  " % (" " * 8, SP1SecDewPress),
                severity=sevf(SP1SecDewPress, 10, 5))
        # if SP2SecDewPress == defstr:
        #     self.logWdg.addMsg("%s sp2LN2: %s;  " % (" " * 8, defstr), )
        # else:
        #     self.logWdg.addMsg(
        #         "%s sp2LN2: %4.1f;  " % (" " * 8, SP2SecDewPress),
        #         severity=sevf(SP1SecDewPress, 10, 5))
        # self.logWdg.addMsg("%s" % ('-' * self.line))
        ...
        #    self.logWdg.addMsg("gcameraTemp= %5.1f (%5.1f)  " % (gcamTemp,
        #    gcamTempN),
        ...
        if vac == defstr:
            ...
            self.logWdg.addMsg(ss, severity=self.redWarn)
        else:
            self.logWdg.addMsg(ss)

        #     ssap="(warning if < 85,  alert if < 75)"
        ssap = "( > 85 )"
        self.logWdg.addMsg("  ln2Level = %s %s" % (ln2Lev, ssap),
                           severity=sevLevL(ln2Lev, 85, 70))
        ...
        if humidPT == defstr:
            ...
            self.logWdg.addMsg("  humid25m =  %s;  " % (str(humidPT)),
                               severity=sevLevU(humidPT, 75, 85))
        airTempPT = sr.getKeyVar(apoModel.airTempPT, ind=0, defVal=defstr)
        dpTempPT = sr.getKeyVar(apoModel.dpTempPT, ind=0, defVal=defstr)
        self.logWdg.addMsg("  airTemp25m =  %s,  dpTemp25m =  %s;  " % (
            str(airTempPT), str(dpTempPT)))
        try:
            diff = airTempPT - dpTempPT
        except Exception:
            diff = defstr
        #  self.logWdg.addMsg("airTemp25m =  %sC,  dpTemp25m =  %sC;  "
        #  % (str(airTempPT), str(dpTempPT)))
        self.logWdg.addMsg("      diff= %s;  " % (str(diff)),
                           severity=sevLevL(diff, 5.0, 2.5))
        ...
        if encl > 0:
            ...
            # ww = str(wind25m)
            # dd = str(dir25m)
            self.logWdg.addMsg("wind25m = %s mph,  direction = %s;  " % (
                str(wind25m), str(dir25m)),
                               severity=sevLevU(wind25m, 30, 35))
        else:
            self.logWdg.addMsg(
                "  wind25m = %s,  direction = %s  " % (defstr, defstr))
    ...
    #      yield sr.waitCmd(actor="boss", cmdStr="status",
# keyVars=[bossModel.sp1Temp,bossModel.sp2Temp], checkFail=False) if
# sr.value.didFail: sp1T="failed"; sp2T="failed" else:
# sp1T=sr.value.getLastKeyVarData(bossModel.sp1Temp)[0]
# sp2T=sr.value.getLastKeyVarData(bossModel.sp2Temp)[0]

...
10-02-2015  Changed enum value for gang position (podium 12)  from int to
    string, based on recent opscore changes 11-30-2020
DG: Removed dithering to match the other version of the APOGEE cals
...
class ScriptClass(object):
    def __init__(self, sr):
        ...
        self.logWdg.addMsg("   {} ".format("  60-reads dark"))
        self.logWdg.addMsg("   {} ".format("  QuartzFlat"))
        self.logWdg.addMsg("   {} ".format("  2 ThAr and UNe arcs"))
        self.logWdg.addMsg("   {} ".format("  60-reads dark"))
    ...
    def run(self, sr):
        ...
        tm = self.getTAITimeStr()[0]
        self.logWdg.addMsg("-- {}-- done --".format(tm), tags=["a"])
...
#    def getTAITimeStr(self,):
#      currPythonSeconds = RO.Astro.Tm.getCurrPySec()
#      self.currTAITuple= time.gmtime(currPythonSeconds

...
class ScriptClass(object):
    ...
    def run(self, sr):
        ...
        self.platedbModel.activePlugging.addCallback(self.updatePlugging,
                                                     callNow=True)
        yield sr.waitCmd(actor="platedb", cmdStr="showActive",
                         abortCmdStr="abort", checkFail=True)
        self.platedbModel.activePlugging.removeCallback(self.updatePlugging)

        n = len(self.ct)
        for i in range(0, n):
            self.logWdg.addMsg(
                "  ct=%2i  pl=%s  --> pl=" % (self.ct[i], self.pl[i]),
                tags=["cur"])
        self.logWdg.addMsg("--------------")

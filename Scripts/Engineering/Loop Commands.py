...
The idea is by KP,  looked at  Run Commands.py as  an example.
...
class ScriptClass(object):
    def __init__(self, sr, ):
        ...
        self.textWdg = RO.Wdg.Text(master=sr.master, width=20, height=5,
                                   ...
                                   borderwidth=1, highlightbackground="grey",
                                   relief="sunken", \
        ...
        self.logWdg = RO.Wdg.LogWdg(master=sr.master, width=20, height=10,
                                    borderwidth=1, highlightbackground="red",
                                    relief="sunken", )
                    ...
                    if params[0].lower() == "wait":
                        if len(params) != 2:
                            raise sr.ScriptError(
                                "tui wait requires one parameter: time (in sec)")
                        waitSec = float(params[1])
                        yield sr.waitMS(waitSec * 1000)
                    else:
                        raise sr.ScriptError(
                            "Unreconized tui command: %r" % (cmdStr,))

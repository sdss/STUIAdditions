"""
02/13/2013  EM  copied from goto6mcpV4-5
10/25/2012  EM  move telescope to desAlt using MCP
11/18/2012   -  changed timeout from 40 sec to 80 sec
01/04/2013   -  changed time rate from 1 sec to 0.4 sec; output every second
    output review of pos and velocity faster by reorganization of check block
    predicted new position by  (velocity)*(time interval) and stop if out of
    range.
01/08/2013   -  call say from subprocess but not from os;  calculate predicted
    value of alt and stop if the next linear destination go below it;  output
    predicted alt; change log style
01/09/2013   -  add +-0.5 degrees behind destination, make room to finish
    naturally
...
2015-11-03  RO  Replace "== None" with "is None" and "!= None" with "is not
    None" to modernize the code.
2019-09-21  EM  new mcp, updated mcp commands and mcp statuses
...
class ScriptClass(object):
    def __init__(self, sr):
        ...
        F1 = tk.Frame(sr.master)
        gr1a = RO.Wdg.Gridder(F1)

        self.lowLimit = 5
        self.altWdg = RO.Wdg.IntEntry(master=F1, defValue=30,
                                      minValue=self.lowLimit, maxValue=90,
                                      helpText="Destination altitude ", )
        ...
        self.ab_I6_L0 = ['alt_mtr_up_perm_in', 'alt_mtr_dn_perm_in',
                         'alt_mtr1_perm_in', 'alt_mtr2_perm_in',
                         'wind_alt_mtr_perm_in', 'alt_plc_perm_in',
                         'wind_alt_plc_perm_in', 'az_stow_3a',
                         'az_mtr_cw_perm_in', 'az_mtr_ccw_perm_in',
                         'az_mtr1_perm_in', 'az_mtr2_perm_in',
                         'wind_az_mtr_perm_in', 'az_plc_perm_in',
                         'wind_az_plc_perm_in', 'az_stow_3b', 'tcc_stop',
                         'cr_stop', 'spare_s1_c2', 'fiber_signal_loss',
                         'n_wind_stop', 'n_fork_stop', 'n_rail_stop',
                         's_rail_stop', 'w_rail_stop', 'n_lower_stop',
                         's_lower_stop', 'e_lower_stop', 'w_lower_stop',
                         's_wind_stop', 'nw_fork_stop', 'mcp_watchdog_timer', ]
        self.ab_I7_L0 = ['alt_grt_83_limit_1', 'bldg_clear_alt', 'az_stow_2a',
                         'az_stow_2b', 'deg_15_stop_ext',
                         'alt_grt_18d6_limit_2', 'alt_slip',
                         'alt_velocity_limit', 'az_dir_cw', 'az_dir_ccw',
                         'az_neg_201a_cw', 'az_pos_445a_ccw', 'az_neg_201b_cw',
                         'az_pos_445b_ccw', 'spare_s8_c6', 'spare_s8_c7',
                         'rot_mtr_cw_perm_in', 'rot_mtr_ccw_perm_in',
                         'rot_mtr_perm_in', 'spare_s5_c3', 'bldg_perm_in',
                         'rot_plc_perm_in', 'hatch_cls', 'alt_les_2d5_limit',
                         'alt_grt_0d3_limit', 'alt_locking_pin_out',
                         'alt_les_90d5_limit', 'bldg_on_alt',
                         'az_109_131_limit_1', 'alt_grt_18d6_limit_1',
                         'az_stow_1a', 'az_stow_1b', ]
        self.ab_I9_L0 = ['s1_c0_bypass_sw', 's1_c1_bypass_sw',
                         's1_c2_bypass_sw', 's1_c3_bypass_sw',
                         's1_c4_bypass_sw', 's1_c5_bypass_sw',
                         's1_c6_bypass_sw', 's1_c7_bypass_sw',
                         's2_c0_bypass_sw', 's2_c1_bypass_sw',
                         's2_c2_bypass_sw', 's2_c3_bypass_sw',
                         's2_c4_bypass_sw', 's2_c5_bypass_sw',
                         's2_c6_bypass_sw', 's2_c7_mcp_wtchdg_byp',
                         't_bar_tel_stat', 'clamp_en_stat', 'clamp_dis_stat',
                         'az_brake_en_stat', 'az_brake_dis_stat',
                         'alt_brake_en_stat', 'alt_brake_dis_stat',
                         'low_lvl_lighting_req', 'solenoid_engage_sw',
                         'alt_locking_pin_in', 'in_9_bit_10_spare',
                         'in_9_bit_11_spare', 'in_9_bit_12_spare',
                         'in_9_bit_13_spare', 'in_9_bit_14_spare',
                         'in_9_bit_15_spare', ]
    ...
    def ifBrakesOn(self, ):
        ...
        alt_brake = self.getBit(self.ab_I9_L0, "alt_brake_en_stat",
                                self.mcpModel.ab_I9_L0[0])
        return alt_brake

    def ifAzStow1(self, ):
        """ check if az position in stow 121 ? """
        az_stow1a = self.getBit(self.ab_I7_L0, "az_stow_1a",
                                self.mcpModel.ab_I7_L0[0])
        az_stow1b = self.getBit(self.ab_I7_L0, "az_stow_1b",
                                self.mcpModel.ab_I7_L0[0])
        return az_stow1a and az_stow1b

    # @staticmethod
    # def getBit1(key, name, val):
    ...
    def run(self, sr):
        ...
        # self.owMcp="(telnet):-1:"
        # owMcpGui="observer@sdss4-hub:mcpop:"
        # owMcpGui="observer@sdsshost2.apo.nmsu.edu"  # previous season

        self.prnMsg("semaphoreOwner = %s" % self.semOwner())

        if self.owMcpGui in self.semOwner():  # "observer@sdss4-hub:mcpop:"
            raise sr.ScriptError(
                " please release MCP GUI semaphore and run again")
        elif self.owTcc in self.semOwner():  # "TCC:0:0"
            pass
        elif self.owMcp in self.semOwner():  # "hub:hub:"
            pass
        else:
        ...
        yield sr.waitCmd(actor="tcc", cmdStr="axis status",
                         keyVars=[self.tccModel.altStat, self.tccModel.azStat,
                                  self.tccModel.rotStat,
        ...
        self.rot = sr.value.getLastKeyVarData(self.tccModel.rotStat)[0]
        self.prnMsg(
            "az=%6.2f,  alt=%5.2f,  rot=%6.2f" % (self.az, self.alt, self.rot))
        if (self.az is None) or (self.alt is None) or (self.rot is None):
        ...
        # it owner is TCC,  run "tcc axis stop" to release  ownership to None  
        # if self.owTcc in self.semOwner():
        ...
        #       yield sr.waitMS(500)
        #       it=0
        #       while self.owNone not in self.semOwner() :
        #           yield sr.waitMS(500)
        #           it=it+1
        #           if it > 10: 
        #                raise sr.ScriptError("tcc axis stop - failed, exit") 
        #       if self.owNone not in self.semOwner():
        #            self.prnMsg("%s   %s .." % (self.semOwner(), self.owNone))
        #            raise sr.ScriptError("tcc axis stop - failed, exit") 

        # it owner is None,  "mcp sem_take" (new sem_steal) will take sem to
        # mcp

        self.prnMsg("1-%s  " % self.semOwner())
        ...
        if self.owTcc in self.semOwner():
            ...
            if self.run2:
                yield sr.waitCmd(actor=act, cmdStr=cmd,
                                 keyVars=[self.mcpModel.semaphoreOwner], )
                it = 0
        ...
        self.prnMsg("%s " % self.semOwner())

        # check, is semOwner in owMcp="hub:hub:"  
        if self.owMcp not in self.semOwner():
        ...
        cmd = "alt goto_pos_va %s 300000 20000" % (
                self.altDes * 3600. / 0.01400002855)
    ...
    def end(self, sr):
        """Clean up"""
        # return semaphore from mcp to tcc
        if self.owMcp in self.semOwner():
            act = "mcp"
            cmd = "sem_give"
            self.prnMsg("end: %s  %s .." % (act, cmd))
